...
public class MInvoice extends X_C_Invoice implements DocAction
{
		...
		String trxName, boolean setOrder)
	{
		...
		// afalcone Start
		// Setea DocumentNo con el mismo numero del documento que anula, agregando en Prefijo una "R" para indicar que esta revertido.
	
		Integer lar_Prefijo = from.get_ValueAsInt("LAR_Prefijo");
		String sPrefijo = "0000"+lar_Prefijo.toString().trim();
		sPrefijo = sPrefijo.substring(sPrefijo.length() - 4);
		sPrefijo = "R"+sPrefijo.substring(sPrefijo.length() - 3);
			
		Integer lar_NroDoc  = from.get_ValueAsInt("LAR_NroDoc");
		String sNroDoc = "00000000"+lar_NroDoc.toString().trim();
		sNroDoc = sNroDoc.substring(sNroDoc.length() - 8);
		to.set_ValueNoCheck ("DocumentNo", sPrefijo+"-"+sNroDoc);
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	public boolean reverseCorrectIt()
	{
		...
		// afalcone start: Setea DocumentNo del nuevo registro
		reversal.setDocumentNo("R"+getDocumentNo());
		
		
//		ver de completar el nro. con los ceros... hay que ver que sucede cuando el documento a revertir ya viene con los 0
