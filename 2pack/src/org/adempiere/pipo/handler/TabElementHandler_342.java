...
public class TabElementHandler_342 extends AbstractElementHandler {
	...
	public void startElement(Properties ctx, Element element) throws SAXException {
		...
		if (isProcessElement(ctx, entitytype)) {
			...
			if (id <= 0 && atts.getValue("AD_Tab_ID") != null && Integer.parseInt(atts.getValue("AD_Tab_ID")) <= PackOut_342.MAX_OFFICIAL_ID)
	...
	throws SAXException {
		...
		String sql = "SELECT * FROM AD_FIELD WHERE AD_TAB_ID = " + AD_Tab_ID + where
			+ "ORDER BY SEQNO asc";
		PreparedStatement pstmt = null;
		pstmt = DB.prepareStatement (sql, getTrxName(ctx));		
		try {									
	...
	{        
		...
		String name = null;
		atts.clear();
		if (m_Tab.getAD_Tab_ID() <= PackOut_342.MAX_OFFICIAL_ID)
