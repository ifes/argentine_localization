...
import org.adempiere.pipo.PackOut_342;
...
public class WindowElementHandler_342 extends AbstractElementHandler {

	private TabElementHandler_342 tabHandler = new TabElementHandler_342();
	...
	public void startElement(Properties ctx, Element element)
		...
		if (isProcessElement(ctx, entitytype)) {
			...
			if (id <= 0 && atts.getValue("AD_Window_ID") != null && Integer.parseInt(atts.getValue("AD_Window_ID")) <= PackOut_342.MAX_OFFICIAL_ID)
	...
	public void create(Properties ctx, TransformerHandler document)
		...
		PackOut_342 packOut = (PackOut_342) ctx.get("PackOutProcess");
	...
	throws SAXException {
		...
		// Tab Tag
		String sql = "SELECT * FROM AD_TAB WHERE AD_WINDOW_ID = "
				+ AD_Window_ID + " And AD_TAB_ID IN "+ where; //faaguilar custom
		PreparedStatement pstmt = null;
		pstmt = DB.prepareStatement(sql, getTrxName(ctx));
		try {
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				...
				String tablename = DB.getSQLValueString(null, tableSql,
						table_id);
				packOut.createTable(rs.getInt("AD_Table_ID"), document,entityType);//faaguilar custom
				createTab(ctx, document, rs.getInt("AD_Tab_ID"),entityType);
			}
			rs.close();
			pstmt.close();
			pstmt = null;
	...
	//faaguilar custom begin
	private void createTab(Properties ctx, TransformerHandler document,
			int AD_Tab_ID, String entityType) throws SAXException {
		Env.setContext(ctx, X_AD_Tab.COLUMNNAME_AD_Tab_ID, AD_Tab_ID);
		tabHandler.create(ctx, document,entityType);
		ctx.remove(X_AD_Tab.COLUMNNAME_AD_Tab_ID);
	}
	//faaguilar custom end
	...
	private AttributesImpl createWindowBinding(AttributesImpl atts,
		...
		atts.clear();
		if (m_Window.getAD_Window_ID() <= PackOut_342.MAX_OFFICIAL_ID)
