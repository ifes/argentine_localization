...
public class TableElementHandler_342 extends AbstractElementHandler {
	...
	public void startElement(Properties ctx, Element element) throws SAXException {
		...
		if (isProcessElement(ctx, entitytype)) {
			...
			if (id <= 0 && atts.getValue("AD_Table_ID") != null && Integer.parseInt(atts.getValue("AD_Table_ID")) <= PackOut_342.MAX_OFFICIAL_ID)
	...
	//begin faaguilar custom
	public void create(Properties ctx, TransformerHandler document, String entityType)
	throws SAXException {
		...
		boolean exported = isTableProcess(ctx, AD_Table_ID);
		AttributesImpl atts = new AttributesImpl();
		String where=" And 1=1";//faaguilar custom
		if(entityType!=null)//faaguilar custom
			where=" And entitytype='"+entityType+"'";//faaguilar custom
		//Export table if not already done so
		if (!exported){
	...
	private AttributesImpl createTableBinding( AttributesImpl atts, X_AD_Table m_Table) 
	{
		...
		String name = null;        
		atts.clear();
		if (m_Table.getAD_Table_ID() <= PackOut_342.MAX_OFFICIAL_ID)
